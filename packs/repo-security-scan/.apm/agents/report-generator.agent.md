...
description: "Collates verified security skill assessment findings and generates a comprehensive vulnerability report written to .sss/ - Brought to you by microsoft/hve-core"
tools:
...
1. Create the `.sss` directory if it does not exist.
...
2. Search `.sss/{REPORT_DATE}` for existing files matching the selected pattern where `{REPORT_DATE}` is the provided report date.
...
1. Select the report format and filename pattern based on mode:
   * When mode is `audit`: assemble the report following VULN_REPORT_V1 and write to `.sss/{REPORT_DATE}/security-report-{NNN}.md`.
   * When mode is `diff`: assemble the report following VULN_REPORT_V1 with diff mode qualifiers and write to `.sss/{REPORT_DATE}/security-report-diff-{NNN}.md`.
   * When mode is `plan`: assemble the report following PLAN_REPORT_V1 and write to `.sss/{REPORT_DATE}/plan-risk-assessment-{NNN}.md`.
